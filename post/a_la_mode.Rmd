...
A few times, I found myself needing a simple function to return the mode. Here are three simple options, first tested on numeric input:

```{r}
library(magrittr)

my_mode_1 <- function(x) {
	unique(x)[unique(x) %>% match(x,.) %>% tabulate %>% which.max]
}
...
my_mode_3 <- function(x) {
	which.max(sapply(1:max(x), function(n) {length(which(x==n))}))
}
```

```{r}
x <- c(5,5,3,2,4,1,2,5,2)

my_mode_1(x)
my_mode_2(x)
my_mode_3(x)
```
If there is a tie, the first two functions return the *first* tied value, where the last function usually returns the numerically lower value.

And on factors:
```{r}
x <- factor(c("e","d","d","a","e","b","a","b","d","b"))

my_mode_1(x)
my_mode_2(x)
# my_mode_3 won't work on factors, as it uses max()
```
