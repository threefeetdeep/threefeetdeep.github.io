...
### Precomputed computations in the ROM
TBD
how the speccy uses the precomputed coeffiecients (12,8,6,12) for EXP,LN,SIN,ARCTAN
how the speccy handles the range of the function to keep the approximation valid? -1 to +1???
...
hex_to_float <- function(hex_in) {
  # strip first byte as exponent
  exponent <- as.integer(as.hexmode(str_sub(hex_in,1,2)))
  
  # strip last four bytes as mantissa, but in two-byte chunks
  # because as.hexmode() range limited to 16-bits
  mantissa_high <- as.integer(as.hexmode(str_sub(hex_in,3,6)))
  mantissa_low <- as.integer(as.hexmode(str_sub(hex_in,7,10)))
  # combine to four bytes
  mantissa <- mantissa_low + 2^16*mantissa_high
  
  # get sign most significant bit of mantissa
  msign <- 1*(mantissa_high <= 2^15)
  
  # calculate output float, handling the sign in the mantissa
  (2*msign-1) * 2^(128-exponent) * (mantissa+2^31*msign)/2^32
}

float_to_hex <- function(float_in) {
  sign <-  0
  # can't handle zero
  if (float_in == 0.0) stop("can't handle zero in float_to_hex()")
  # take out sign
  if (float_in < 0) {
    sign <- 1
  }
  
  # convert input in powers of ten of powers of two
  exp <- floor(log2(10) * log10(float_in)) + 1
  
  # create fractional mantissa part
  mantissa <- 2^(32-exp)*float_in
  
  if (sign == 0) {
    # set MSB to be zero for positive
    mantissa <- mantissa - 2^31
  }

  exp_hex <- int_to_hex(exp, 1)
  mantissa_hex <- int_to_hex(mantissa, 4)
  return(paste0(exp_hex,mantissa_hex))
}


int_to_hex <- function(i, n) {
  # needed to handle large ints as R can only handle 16-bits
  # n is number of bytes to output (masked from lowest)
  high_word <- floor(i/2^16)
  low_word <- i - high_word * 2^16
  
  high_hex <- format(as.hexmode(high_word), width=4)
  low_hex <- format(as.hexmode(low_word), width=4)
  
  combined <- paste0(high_hex, low_hex)
  N <- str_length(combined)
  start_idx <- ifelse(N - 2*n + 1 < 1,1,N - 2*n + 1)
  end_idx <- N
  return(str_sub(combined,start_idx, end_idx))
}
``` 
...
format(lapply(sin_coefs,FUN=hex_to_float), scientific = T)
...
* An [article](https://aip.scitation.org/doi/pdf/10.1063/1.4823278) from "Computers in Physics" 1994 that includes an approximation exmaple for $e^x$
