...
Note that outside this limited range for $x$, these polynomials generally go crazy dissapearing off to extreme values, so we really must stick with them only within our limited range.
...
However, it is easier just to add upp the Chebyshev polynomials directly, and this is how the ZX Spectrum does it.
...
### End Notes
There is more information about the exact method used by teh ZX Spectrum in the fantastic (but totally out of print) book by Logan and O'Hara. You can get a copy for peanuts on Ebay!
