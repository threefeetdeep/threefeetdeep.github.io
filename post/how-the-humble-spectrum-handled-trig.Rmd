...
Four of these are "fundamental" operators, used to derive the others:

**EXP &ensp; LN &ensp; SIN &ensp; ARCTAN**

The derivations for the others are shown below. Some are simple, but quite crafty conversions!
...
**POW** &emsp; $x^y = e^{y \cdot ln(x)} = e^{ln(x^y)}$
...
By using only four operators to derive a total of ten, the Speccy team we're cleverly able to keep memory usage low, at the cost of slower speed with multiple conversions using only the four "base" operators.
<hr>
## Where the magic happens: Chebyshev Approximations
We have just seen how the ZX Spectrum handles ten functions for the price of only four. The question is, how does it actually implement these functions using the simple-minded Z80 CPU and a tiny amount of ROM?

Step forward the Chebyshev Polynomial Series approximation!!


### Some theory
TBD.  Intro to Chebyshev polynomial approximation.
